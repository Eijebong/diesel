...
macro_rules! __diesel_column {
    ($($table:ident)::*, $column_name:ident -> ($($Type:tt)*),  $real_name:expr, $($doc:expr),*) => {
        $(
            #[doc=$doc]
        )*
...
/// the `real_name` attribute like this:
///
/// ```
/// #[macro_use] extern crate diesel;
///
...
macro_rules! table_body {
    ...
    (
        ...
        columns = [$($column_name:ident -> $Typey:tt; doc = [$($doc:expr)*]; real_name = $real_name:expr,)*],
        imports = ($($($import:tt)::+),+),
        table_doc = [$($table_doc:expr)*],
        current_column_doc = [$($column_doc:expr)*],
        current_column_real_name = [$($current_column_real_name:expr)*],
        #[doc=$new_doc:expr]
        $($body:tt)*
    ) => {
        table_body! {
            ...
            columns = [$($column_name -> $Typey; doc = [$($doc)*]; real_name = $real_name,)*],
            imports = ($($($import)::+),+),
            table_doc = [$($table_doc)*],
            current_column_doc = [$($column_doc)*$new_doc],
            current_column_real_name = [$($current_column_real_name)*],
            $($body)*
        }
    };

    // Parse the real_name attribute and forward the remaining table body to further instances of
    // this macro
    (
        schema_name = $schema_name:ident,
        table_name = $name:ident,
        primary_key_ty = $primary_key_ty:ty,
        primary_key_expr = $primary_key_expr:expr,
        columns = [$($column_name:ident -> $Type:tt; doc = [$($doc:expr)*]; real_name = $real_name:expr,)*],
        imports = ($($($import:tt)::+),+),
        table_doc = [$($table_doc:expr)*],
        current_column_doc = [$($column_doc:expr)*],
        current_column_real_name = [],
        #[real_name=$new_real_name:expr]
        $($body:tt)*
    ) => {
        table_body! {
            schema_name = $schema_name,
            ...
            primary_key_ty = $primary_key_ty,
            primary_key_expr = $primary_key_expr,
            columns = [$($column_name -> $Type; doc = [$($doc)*]; real_name = $real_name,)*],
            imports = ($($($import)::+),+),
            table_doc = [$($table_doc)*],
            current_column_doc = [$($column_doc)*],
            current_column_real_name = [$new_real_name],
            $($body)*
        }
    };
    ...
    (
        ...
        table_doc = [$($table_doc:expr)*],
        current_column_doc = [$($column_doc:expr)*],
        current_column_real_name = [$new_real_name:expr],
        ...
        table_body! {
            ...
            primary_key_expr = $primary_key_expr,
            columns = [$($column_name -> $Type; doc = [$($doc)*]; real_name = $real_name,)*
                       $new_column_name -> ($($ty_path)::*$(<$($ty_params)::*>)*); doc = [$($column_doc)*]; real_name = $new_real_name,],
            imports = ($($($import)::+),+),
            table_doc = [$($table_doc)*],
            current_column_doc = [],
            current_column_real_name = [],
    ...
    (
        ...
        primary_key_ty = $primary_key_ty:ty,
        primary_key_expr = $primary_key_expr:expr,
        columns = [$($column_name:ident -> $Typea:tt; doc = [$($doc:expr)*]; real_name = $real_name:expr,)*],
        imports = ($($($import:tt)::+),+),
        table_doc = [$($table_doc:expr)*],
        current_column_doc = [$($column_doc:expr)*],
        current_column_real_name = [$new_real_name:expr],
        ...
        table_body! {
            ...
            primary_key_ty = $primary_key_ty,
            primary_key_expr = $primary_key_expr,
            columns = [$($column_name -> $Typea; doc = [$($doc)*]; real_name = $real_name,)*
                       $new_column_name -> ($new_column_ty); doc = [$($column_doc)*]; real_name = $new_real_name,],
            imports = ($($($import)::+),+),
            table_doc = [$($table_doc)*],
            current_column_doc = [],
            current_column_real_name = [],
    ...
    // Add a real_name arg if we find a column definition without any real_name attribute before
    (
        schema_name = $schema_name:ident,
        table_name = $name:ident,
        primary_key_ty = $primary_key_ty:ty,
        ...
        table_body! {
            ...
            table_doc = [$($table_doc)*],
            current_column_doc = [$($column_doc)*],
            current_column_real_name = [stringify!($new_column_name)],
    ...
    (
        ...
        primary_key_ty = $primary_key_ty:ty,
        primary_key_expr = $primary_key_expr:expr,
        columns = [$($column_name:ident -> ($($column_ty:tt)*); doc = [$($doc:expr)*]; real_name = $real_name:expr,)+],
        imports = ($($($import:tt)::+),+),
        table_doc = [$($table_doc:expr)*],
        current_column_doc = [],
        current_column_real_name = [],
        ...
        pub mod $table_name {
            ...
            pub mod columns {
                ...
                $(__diesel_column!(table, $column_name -> ($($column_ty)*), $real_name, $($doc),*);)+
            }
        }
    }
}
